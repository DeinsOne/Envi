...
namespace Envi {
    ...
    ENVI_EXTERN KeyCode ConvertToNative(Envi::KeyCodes key);
    ENVI_EXTERN Envi::KeyCodes ConvertToKeyCode(KeyCode key);

    template <>
    void SendInput<KeyEvent, std::chrono::milliseconds>(const KeyEvent& e, const std::chrono::milliseconds& time) {
        Envi::Timer timer = Envi::Timer(time);
    ...
    void SendInput<MousePositionOffsetEvent, std::chrono::milliseconds>(const MousePositionOffsetEvent& e, const std::chrono::milliseconds& time) {
        ...
        if (tm == 0) {
            auto display = XOpenDisplay(NULL);
            XTestFakeRelativeMotionEvent(display, e.X, e.Y, CurrentTime);
            XCloseDisplay(display);
            return;
        }
        ...
        for (int i = 0; i < tm / _tTime; i++) {
            Envi::Timer timer = Envi::Timer(std::chrono::milliseconds(_tTime));
            auto display = XOpenDisplay(NULL);
            XTestFakeRelativeMotionEvent(display, xStep, yStep, CurrentTime);
            XCloseDisplay(display);
            timer.wait();
        }

        auto display = XOpenDisplay(NULL);
        XTestFakeRelativeMotionEvent(display, (e.X * _tTime) % tm, (e.Y * _tTime) % tm, CurrentTime);
        XCloseDisplay(display);
    }
    ...
    Envi::KeyCodes ConvertToKeyCode(KeyCode key) {
        switch(key) {
        case 9:
